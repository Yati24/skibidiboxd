...
<div class="flex flex-wrap gap-2 mb-8">
	<button
		...
		class="px-3 py-1.5 text-sm rounded-lg transition-colors {selectedCategory === null ? 'bg-cyan-600 text-white' : 'bg-slate-800 text-slate-300 hover:bg-slate-700'}"
	>
		Toutes
	</button>
	{#each data.categories as category}
		<button
			...
			class="px-3 py-1.5 text-sm rounded-lg transition-colors {selectedCategory === category ? 'bg-cyan-600 text-white' : 'bg-slate-800 text-slate-300 hover:bg-slate-700'}"
		>
			{category}
		</button>
	{/each}
